    ...
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-5xl mx-auto">
            ...
            <div className="space-y-2">
              {stats.recentActivity.map((item) => (
                <div key={item.id} className="flex items-center justify-between rounded-lg border border-border/50 px-4 py-2">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full ${item.status === "completed" ? "bg-emerald-500/10 text-emerald-600" : item.status === "failed" ? "bg-red-500/10 text-red-600" : "bg-amber-500/10 text-amber-600"}`}>
                    {item.status}
                  </span>
                </div>
              ))}
        ...
        <div className="rounded-2xl border border-border p-6">
          ...
          <button className="rounded-lg bg-primary text-primary-foreground px-4 py-2 text-sm font-medium hover:bg-primary/90">Generate New Key</button>
        </div>
      </div>
    </div>
  );
